...
void Game::CreateEnemy(Vector2 pos, float W, float H, Color c, float Health, float Attack, float t, Type_attack type) {
	Enemys.push_back(std::make_unique<Enemy>(pos, W, H, c, Health, Attack, t, type));
}
...
void Game::SetPlayer(Vector2 pos, float W, float H, Color c, float health, int exp) {
